...
export default function LocationMap({ lat, lon, accent, current }) {
  ...
  // OpenStreetMap embed — completely free, no API key needed
  ...
  const stats = [
    { label: 'Latitude',    value: lat.toFixed(4),  icon: '📍' },
    { label: 'Longitude',   value: lon.toFixed(4),  icon: '📍' },
    { label: 'Timezone',    value: `UTC${current.timezone >= 0 ? '+' : ''}${Math.round(current.timezone / 3600)}`, icon: '🕐' },
      ...
      <div style={{
        ...
        {!mapLoaded && (
          <div style={{
            ...
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>Loading map...</div>
          </div>
        )}
        ...
        {/* Gradient overlay top for blend */}
        <div style={{
